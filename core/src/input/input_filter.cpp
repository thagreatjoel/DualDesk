...
bool InputFilter::ShouldProcessEvent(const InputEvent& event) const {
    if (!workspaceManager_) {
        LOG_WARN("No WorkspaceManager set, allowing all events");
        return true;
    }

    // Check device assignment
    if (!IsDeviceInActiveWorkspace(event.deviceId)) {
        LOG_DEBUG("Device {} not in active workspace, filtering event", event.deviceId);
        return false;
    }

    // Filter mouse movement if enabled
    if (filterMouseMovement_ && event.type == InputEventType::MouseMove) {
        return false;
    }

    // Filter system keys if enabled
...
Workspace* InputFilter::GetTargetWorkspace(const InputEvent& event) const {
    ...
    // Check if device belongs to active workspace
    if (active->GetKeyboardId() == event.deviceId ||
        active->GetMouseId() == event.deviceId) {
        return active;
    }

    // Check if device belongs to any workspace
    ...
    // If device isn't assigned to any workspace, use active
    return active;
}
